...
type SendTo = "group" | "class" | "student" | "all_teachers" | "teacher";
type RecipientType = "group" | "grade" | "student" | "all_teachers" | "teacher";
...
export default function AdminNotificationsPage() {
  ...
  // ✅ استخدام getActiveGrades بدلاً من getGrades
  ...
  // ✅ التأكد من أن gradesList تحتوي على البيانات الصحيحة
  ...
  const recipientCount = sendTo === "group"
    ? (selectedGroup?.currentStudents ?? selectedGroup?.students?.length ?? 0)
    : sendTo === "all_teachers" ? teacherList.length
      : 1;
  ...
  // ✅ تحويل SendTo إلى RecipientType
  ...
  // ✅ الحصول على recipientId المناسب
  const getRecipientId = (): Id<"users"> | Id<"groups"> | Id<"grades"> | undefined => {
    if (sendTo === "group") {
      return groupId as Id<"groups">;
    }
    if (sendTo === "class") {
      return classId as Id<"grades">;
    }
    if (sendTo === "student") {
      return studentId as Id<"users">;
    }
    if (sendTo === "teacher") {
      return teacherId as Id<"users">;
    }
    return undefined;
  };

  // ✅ دالة للحصول على اسم الصف المختار
      ...
      <header className="bg-linear-to-r from-[#001f24] to-[#03363d] px-8 py-5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-[#a3ced6] text-sm mt-0.5">إرسال وإدارة إشعارات الطلاب والمعلمين</p>
          </div>
                  ...
                  <div>
                    ...
                    <Dropdown
                      ...
                      options={gradesList.map((g: any) => ({ 
                        value: g._id, 
                        label: g.name || g.nameEn || g._id 
                      }))}
                    />
                    {/* ✅ عرض اسم الصف المختار بدلاً من ID */}
                    {classId && (
                      <p className="flex items-center gap-1.5 text-xs text-[#1a7a8a] mt-2">
            ...
            <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
              ...
              <div className="p-5">
                ...
                <div className="flex items-center justify-end gap-1.5 mt-2 text-xs text-gray-400">
                  <span>المستلمون: {recipientCount} طالب</span>
                  <Users className="h-3.5 w-3.5" />
                </div>
              </div>
            </div>
