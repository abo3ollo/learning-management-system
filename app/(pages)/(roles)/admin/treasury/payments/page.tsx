// app/(pages)/(roles)/admin/treasury/payments/page.tsx
"use client";
...
export default function PaymentVouchersPage() {
  ...
  // ── Queries ───────────────────────────────────────────────────
  const vouchers = useQuery(api.treasury.treasury.getPaymentVouchers, {});
  const currentUser = useQuery(api.user.auth.getCurrentUser);

  // ── Mutations ─────────────────────────────────────────────────
  const createVoucher = useMutation(api.treasury.treasury.createPaymentVoucher);
  const updateVoucher = useMutation(api.treasury.treasury.updatePaymentVoucher);
  const deleteVoucher = useMutation(api.treasury.treasury.deletePaymentVoucher);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const data = {
        payeeId: currentUser._id as Id<"users">,
  ...
  const handleDelete = async (voucherId: string) => {
    ...
    try {
      await deleteVoucher({ voucherId: voucherId as Id<"paymentVouchers"> });
      ...
      printWindow.document.write(`
        <html dir="rtl">
          ...
          <body>
            ...
            <div class="info"><span class="label">المدفوع له:</span> ${voucher.payeeName}</div>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">إدارة سندات الصرف المالية</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          {filteredVouchers.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-gray-400">لا توجد سندات صرف</p>
            </div>
          ) : (
            <>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">المدفوع له *</label>
              <Input
