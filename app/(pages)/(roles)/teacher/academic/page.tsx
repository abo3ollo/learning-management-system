// app/(pages)/(roles)/teacher/academic/page.tsx
"use client";
...
import { AddAcademicMaterialModal } from "@/app/_components/Teacher/AddAcademicMaterialModal";
import { AcademicCoursePriceModal } from "@/app/_components/Teacher/AcademicCoursePriceModal";
...
export default function TeacherAcademicPage() {
    ...
    // ✅ استخدم academicMaterials بدلاً من teacherMaterials
    const materials = useQuery(
        api.teacherMaterials.teacherMaterials.getTeacherAcademicMaterials,
        currentUser?._id ? { teacherId: currentUser._id as any } : "skip"
    );

    const deleteMaterial = useMutation(api.teacherMaterials.teacherMaterials.deleteAcademicMaterial);
    ...
    return (
        <div className="min-h-screen bg-[#f7fafa]" dir="rtl">
            ...
            <header className="bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between sticky top-0 z-10">
                <div>
                    <h1 className="text-xl font-semibold text-[#001f24]">موادي التحصيلية</h1>
                    <p className="text-sm text-gray-500">إدارة المواد التحصيلية الخاصة بك</p>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    >
                        ...
                        {currentUser?.academicCoursePrice ? (
                            <span>{currentUser.academicCoursePrice} {currentUser.academicCourseCurrency || "EGP"}</span>
                        ) : (
                            "تحديد سعر الكورس"
                        )}
                    </Button>
            ...
            {/* Add/Edit Material Modal - استخدام مكون خاص بالتحصيلي */}
            <AddAcademicMaterialModal
            ...
            <AcademicCoursePriceModal
                isOpen={isCoursePriceModalOpen}
                onClose={() => setIsCoursePriceModalOpen(false)}
                teacherId={currentUser?._id}
                initialPrice={currentUser?.academicCoursePrice || 0}
                initialCurrency={currentUser?.academicCourseCurrency || "EGP"}
