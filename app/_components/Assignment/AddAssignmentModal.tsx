...
export function AddAssignmentModal({ isOpen, onClose, editAssignmentId }: AddAssignmentModalProps) {
    // ✅ جلب البيانات
    const courses = useQuery(api.courses.courses.getCourses, {}); // ✅ المواد
    const classes = useQuery(api.classes.classes.getClasses, {});     // ✅ الفصول
    ...
    const [formData, setFormData] = useState({
        ...
        description: "",
        courseId: "",          // ✅ مادة واحدة
        classIds: [] as string[], // ✅ فصول متعددة
        type: "assignment",
        maxAttempts: "",
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <select
                                    ...
                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1a7a8a] bg-white ${errors.subjectId ? "border-red-500" : "border-[#c0c8c9]"
                                        }`}
                                >
                                    <option value="">اختر المادة</option>
                                    {courses?.map((courses: any) => (
                                        <option key={courses._id} value={courses._id}>
                                            {courses.title || courses.subjectName || "مادة"}
                                        </option>
                                    ))}
                                </select>
                                {errors.subjectId && (
                                    <p className="text-xs text-red-500 flex items-center gap-1">
                                        <AlertCircle className="h-3 w-3" /> {errors.subjectId}
                                    </p>
                                )}
                                {courses?.length === 0 && (
