// app/_components/AddCourseModal.tsx
"use client";
...
export function AddCourseModal({ isOpen, onClose }: AddCourseModalProps) {
  ...
  const createCourse = useMutation(api.courses.courses.createCourse);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // هنا يمكن رفع الصورة إلى خدمة تخزين
      ...
      setErrors({ submit: "حدث خطأ أثناء إضافة المادة" });
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-[#c0c8c9] px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-[#001f24]">إضافة مادة جديدة</h2>
            <p className="text-sm text-gray-500 mt-1">أدخل تفاصيل المادة الدراسية</p>
          </div>
          <button
            onClick={onClose}
