// app/_components/Teacher/AcademicCoursePriceModal.tsx
...
import { X, Loader2, DollarSign, Save, AlertCircle, GraduationCap } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";

interface AcademicCoursePriceModalProps {
  isOpen: boolean;
  onClose: () => void;
  teacherId: string;
  initialPrice?: number;
...
export function AcademicCoursePriceModal({
  isOpen,
  onClose,
  teacherId,
  initialPrice = 0,
  initialCurrency = "EGP",
  teacherName = "",
}: AcademicCoursePriceModalProps) {
  const [price, setPrice] = useState<string>("");
  const [currency, setCurrency] = useState<string>("EGP");
  const [error, setError] = useState<string | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);

  // ✅ المسار الصحيح: api.teacher.coursePrice.updateAcademicCoursePrice
  const updateCoursePrice = useMutation(api.coursePrice.coursePrice.updateAcademicCoursePrice);
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-xl">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">سعر الكورس التحصيلي</h2>
              {teacherName && (
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-3">
            <div className="flex items-start gap-2 text-sm text-blue-700">
              ...
              <div>
                <p className="font-medium">سعر الكورس التحصيلي</p>
                <p className="text-xs mt-1">
                  هذا السعر يشمل <strong>جميع المواد التحصيلية</strong> التي يقدمها هذا المعلم
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input
                ...
                placeholder="أدخل سعر الكورس التحصيلي"
                className="pr-12 text-lg font-medium border-gray-200 focus:border-blue-500 focus:ring-blue-500"
                required
                autoFocus
              />
            ...
            <p className="text-xs text-gray-400 mt-2">
              💡 هذا السعر سيظهر للطلاب عند شراء الكورس التحصيلي
            </p>
          </div>
