// app/_components/Teacher/AddExamModalTeacher.tsx
...
export function AddExamModalTeacher({ isOpen, onClose, editExamId, onSuccess }: AddExamModalProps) {
  // ✅ جلب مجموعات المعلم فقط (التي هو مشرف أو معلم فيها)
  ...
  // ✅ في handleSubmit - الحل الأول
  const handleSubmit = async (status: "draft" | "published") => {
    if (!validateForm()) return;

    setIsSubmitting(true);
    try {
      // ✅ الحصول على أول مجموعة مختارة
      const firstGroupId = formData.groupIds[0];
      const selectedGroup = teacherGroups?.find((g: any) => g._id === firstGroupId);
      
      // ✅ الحصول على gradeId من المجموعة المختارة
      let gradeId = selectedGroup?.gradeId;
      
      // ✅ إذا لم يكن هناك gradeId في المجموعة المختارة، حاول البحث عن أي مجموعة تحتوي على gradeId
      if (!gradeId && teacherGroups) {
      ...
      // ✅ اسم الصف من المجموعة المختارة أو من النموذج
      const gradeName = selectedGroup?.gradeName || formData.grade || "غير محدد";

      // ✅ بناء البيانات مع gradeId (مطلوب)
      const data = {
        title: formData.title,
        description: formData.description || undefined,
        model: formData.model || `النموذج ${String.fromCharCode(65 + Math.floor(Math.random() * 26))}`,
        grade: gradeName,
        gradeId: gradeId as any, // ✅ إرسال gradeId (مطلوب)
  ...
  // ✅ قائمة المجموعات المتاحة للمعلم
  const availableGroups = teacherGroups || [];
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {activeTab === "basic" && (
            <div className="space-y-4">
              ...
              {/* ✅ المجموعات - اختيار متعدد (مجموعات المعلم فقط) */}
              <div className="space-y-2">
