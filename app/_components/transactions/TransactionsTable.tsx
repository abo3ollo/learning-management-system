// app/_components/transactions/TransactionsTable.tsx

"use client";

import { Eye, CheckCircle, XCircle, Clock, AlertCircle, Video, Award, ShoppingBag, BookOpen } from "lucide-react";
...
const typeIcons = {
  platform: Video,
  aptitude: Award,
  academic: BookOpen, // ✅ أضف academic
...
const typeColors = {
  ...
  academic: "bg-blue-100 text-blue-700", // ✅ أضف academic
  purchase: "bg-green-100 text-green-700",
};

const typeLabels = {
  platform: { ar: "منصة", en: "Platform" },
  aptitude: { ar: "قدرات", en: "Aptitude" },
  academic: { ar: "تحصيلي", en: "Academic" },
      ...
      <table className="w-full">
        ...
        <tbody className="divide-y divide-gray-100">
          {transactions.map((transaction, index) => {
            ...
            const typeLabel = typeLabels[transaction.type as keyof typeof typeLabels] || { ar: transaction.type, en: transaction.type };
            const statusLabel = statusLabels[transaction.status as keyof typeof statusLabels] || { ar: transaction.status, en: transaction.status };

            return (
              <tr key={transaction._id || transaction.uniqueKey || index} className="hover:bg-gray-50 transition-colors">
                <td className="px-6 py-4 text-sm text-gray-500">{index + 1}</td>
                
                {showStudent && (
                  <td className="px-6 py-4">
                    <div>
                      <p className="text-sm font-medium text-gray-700">{transaction.studentName || "غير معروف"}</p>
                ...
                <td className="px-6 py-4">
                  ...
                  {(transaction.isAptitude || transaction.isAcademic) && (
                    <p className="text-xs text-gray-400 mt-1">معلم: {transaction.teacherName}</p>
                  )}
                </td>
                ...
                <td className="px-6 py-4">
                  <p className="text-sm font-medium text-gray-900">
                    {transaction.amount?.toFixed(2) || 0} {transaction.currency || "EGP"}
                  </p>
                </td>
                ...
                <td className="px-6 py-4 text-sm text-gray-500">
                  {transaction.createdAt ? new Date(transaction.createdAt).toLocaleDateString("ar-EG") : "-"}
                </td>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    ...
                    {showActions && transaction.status === "pending" && onStatusChange && (
                      <>
                        <button
                          onClick={() => onStatusChange(transaction._id, "approved")}
                        ...
                        <button
                          onClick={() => onStatusChange(transaction._id, "rejected")}
