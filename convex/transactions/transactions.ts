  ...
  handler: async (ctx, args) => {
    ...
    if (args.searchQuery) {
      ...
      transactions = transactions.filter((t) => {
        const studentName = t.studentId?.toString().toLowerCase() || "";
...
export const getChildrenTransactions = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!parent || parent.role !== "parent") {
      throw new Error("غير مصرح: فقط ولي الأمر يمكنه الوصول");
    }
...
export const getTransactionStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.studentId) {
      ...
      // إذا لم يتم تحديد، جلب الكل (للأدمن)
